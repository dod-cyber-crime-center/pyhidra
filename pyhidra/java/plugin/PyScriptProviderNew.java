...
@ExtensionPointProperties(priority = 1001) // Enforce high priority so pyhidra is the default Python provider when installed
public final class PyScriptProvider extends AbstractPythonScriptProvider {
	...
	public String getRuntimeEnvironmentName() {
		// NOTE: using PyGhidra so users will only need to add this once
		return "PyGhidra";
	}
	...
	public GhidraScript getScriptInstance(ResourceFile sourceFile, PrintWriter writer) {
		...
		if (scriptRunner == null || GhidraScriptUtil.isSystemScript(sourceFile)) {
			try {
				return new JythonScriptProvider().getScriptInstance(sourceFile, writer);
			}
			catch (Throwable t) {
